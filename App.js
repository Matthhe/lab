...
function App() {
  ...
  const [userGuess, setUserGuess] = useState(null); // Догадка пользователя
  const [gameStatus, setGameStatus] = useState('idle'); // Статус игры: idle, playing, finished
  const [feedback, setFeedback] = useState(null); // Обратная связь после ответа
  ...
  // Загрузка вопросов при старте приложения
  useEffect(() => {
        ...
        <div className="game-container">
          ...
          <div className="map-container">
            ...
            >
              ...
              {/* Правильный ответ (показываем после ответа пользователя) */}
              {userGuess && (
                <Circle
